(function(){
  ...
  function modelCard(product,model){
    return `<article class="model-card"><div class="model-card-head"><div class="model-card-icon" aria-hidden="true">⚡</div><div><div class="section-kicker">${escapeHtml(model.subcategory||product.category||'')}</div><h3>${escapeHtml(model.model||product.modelName)}</h3></div></div><div class="model-grid">${modelField('Power',model.power)}${modelField('Current',model.current)}${modelField('Voltage',model.voltage)}${modelField('Frequency',model.frequency)}${modelField('Phase',model.phase)}${modelField('IP',model.ip)}${modelField('Insulation',model.insulation)}${modelField('Lumen',model.lumen)}${modelField('Operating Temperature',model.operatingTemperature)}${modelField('Order Code',model.orderCode)}</div><div class="model-card-footer"><span>${escapeHtml(product.category||'')}</span><b>${escapeHtml(model.price||'')}</b></div></article>`;
  }
