(function(){
  ...
  function modelCard(product,model){
    ...
    const identity=model.orderCode||model.model||'';
    return `<article class="model-card"><div class="model-card-head">${visual}<div><div class="section-kicker">${escapeHtml(model.subcategory||product.category||'')}</div><h3>${escapeHtml(model.model||product.modelName)}</h3><div class="model-name">${escapeHtml(model.name||'')}</div></div></div><div class="model-grid">${modelField('Power',model.power)}${modelField('Current',model.current)}${modelField('Voltage',model.voltage)}${modelField('Frequency',model.frequency)}${modelField('Phase',model.phase)}${modelField('IP',model.ip)}${modelField('Insulation',model.insulation)}${modelField('Lumen',model.lumen)}${modelField('Operating Temperature',model.operatingTemperature)}</div><button class="model-project-btn" type="button" data-add-electrical-project="${escapeHtml(identity)}">＋ Add to Project</button><div class="model-card-footer"><span>${escapeHtml(product.category||'')}</span><b>${escapeHtml(model.price||'')}</b></div></article>`;
  }
