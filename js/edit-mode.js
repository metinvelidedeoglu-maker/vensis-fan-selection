(function(){
  ...
  function showSessionPanel(){
    ...
    const content=isCatalogPage()
      ...
      : `${head('Project Cloud active','Projects and customers are synchronized securely across your devices.')}<div class="vensis-edit-body">${storageWarning}<div class="vensis-edit-status"><span class="vensis-edit-status-dot"></span><div><b>Secure session is open</b><small>${esc(window.VensisCustomers?.cloudState?.().message||window.VensisProjects?.cloudState?.().message||'Projects and customers are stored outside public_html on Hostinger.')}</small></div></div><p class="vensis-edit-note">Browser copies remain available for speed. The protected Hostinger copy restores them on another signed-in device.</p><div class="vensis-edit-actions"><button id="vensisEditLogout" class="vensis-edit-danger" type="button">Sign out</button><button class="vensis-edit-primary" type="button" data-edit-close>Continue</button></div></div>`;
