(function(){
  ...
  function setLanguage(lang){
    if(window.VensisI18n?.setLanguage){
      window.VensisI18n.setLanguage(lang);
  ...
  function installStyles(){
    if(document.getElementById('vensisSuiteSidebarStyle'))return;
    const style=document.createElement('style');
    style.id='vensisSuiteSidebarStyle';
    style.textContent=`
      :root{--vensis-side-width:210px;--vensis-side-top:72px}
      ...
      .vensis-side-menu-group-title{display:flex!important;align-items:center!important;justify-content:space-between!important;min-height:42px!important;padding:10px 12px!important;border-radius:9px!important;background:#f2f6f5!important;color:#29484d!important;font-size:13px!important;font-weight:800!important}
      .vensis-side-menu-group-title.active{background:#e7f4ee!important;color:#087f4f!important;border:1px solid #b9d5c8!important}
  ...
  function detectHeaderBottom(){
    const candidates=[...document.body.children].filter(el=>
      el!==document.getElementById('vensisSideMenu') &&
      (el.matches('header')||el.classList.contains('top')||el.classList.contains('catalog-top'))
    );
    const visible=candidates.find(el=>{
      const cs=getComputedStyle(el);
      const r=el.getBoundingClientRect();
      return cs.display!=='none'&&cs.visibility!=='hidden'&&r.height>10;
    });
    const top=visible?Math.max(0,Math.round(visible.getBoundingClientRect().bottom)):0;
    document.documentElement.style.setProperty('--vensis-side-top',`${top}px`);
  }

  function updateLanguageButtons(){
    const lang=window.VensisI18n?.getLanguage?.()||currentLang();
    document.querySelectorAll('#vensisSideMenu [data-side-lang]').forEach(btn=>{
      btn.classList.toggle('active',btn.dataset.sideLang===lang);
    });
  }
  ...
  function mount(){
    ...
    if(existing){detectHeaderBottom();updateLanguageButtons();return existing}
    ...
    menu.innerHTML=`
      ...
      <div class="vensis-side-menu-foot">
        ...
        <div class="vensis-side-lang">
          <button type="button" data-side-lang="en">EN</button>
          <button type="button" data-side-lang="tr">TR</button>
        </div>
      </div>`;

    document.body.appendChild(menu);
    document.body.classList.add('vensis-side-menu-ready');
    menu.querySelectorAll('[data-side-lang]').forEach(btn=>btn.addEventListener('click',()=>setLanguage(btn.dataset.sideLang)));
    updateLanguageButtons();
    detectHeaderBottom();
    requestAnimationFrame(detectHeaderBottom);
    setTimeout(detectHeaderBottom,250);
    return menu;
  }
  ...
  window.addEventListener('resize',()=>{
    ...
    resizeTimer=setTimeout(detectHeaderBottom,80);
  });
  window.addEventListener('vensis-language-changed',updateLanguageButtons);

  window.VensisSuiteShell={mount,build:BUILD,detectHeaderBottom};
