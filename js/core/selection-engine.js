(function(){
  ...
  function select(){
    ...
    const qL=q*(1+U.number('qmin')/100),qH=q*(1+U.number('qmax')/100),pL=p*(1+U.number('pmin')/100),pH=p*(1+U.number('pmax')/100);
    const maxText=U.byId('maxkw')?.value.trim(),maxkw=maxText?Number(maxText):Infinity,pole=U.byId('pole')?.value||'',sort=U.byId('sort')?.value||'closest';
    const out=[];
    for(const model of S.models){
      if(S.selectedTags.size&&![...S.selectedTags].every(t=>(model.tags||[]).includes(t)))continue;
      ...
      for(let pressure=Math.ceil(pL);pressure<=Math.floor(pH);pressure++){
        const flow=U.interpolate(model.points,pressure); if(flow==null||flow<qL||flow>qH)continue;
