(function(){
  ...
  function isLoggedIn(){
    try{
      ...
      return !!(s&&s.username===USERNAME&&s.expiresAt>Date.now());
    }catch(e){return false}
  }
  async function login(username,password){
    const ok=String(username).trim().toLowerCase()===USERNAME && await sha256(String(password))===PASSWORD_HASH;
    if(ok)localStorage.setItem(SESSION_KEY,JSON.stringify({username:USERNAME,expiresAt:Date.now()+7*24*60*60*1000}));
    return ok;
  }
  function logout(){
    localStorage.removeItem(SESSION_KEY);
    localStorage.removeItem('vensis_active_project_v2');
  ...
  function injectLoginButton(){
    ...
    btn.id='authButton';
    btn.textContent=isLoggedIn()?'Çıkış Yap':'Giriş Yap';
    btn.onclick=()=>isLoggedIn()?logout():(location.href='login.html');
  ...
  function applyBodyState(){
    ...
    document.body.classList.toggle('logged-user',isLoggedIn());
  }
