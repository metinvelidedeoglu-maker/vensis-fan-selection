(function(){
  ...
  function curvePoints(points,sourcePoints,options={}){
    const existing=populatedPoints(points);
    if(existing.length)return existing;
    const source=populatedPoints(sourcePoints);
    if(options.precomputed||String(options.interpolation||'').toLowerCase()==='linear')return source;
  ...
  function normalizePoints(points){
    ...
    for(const point of points||[]){
      ...
      const key=`${p}|${q}`;
      if(Number.isFinite(p)&&Number.isFinite(q)&&p>=0&&q>=0&&key!==previousKey){
        result.push({p,q});
        previousKey=key;
      }
    }
    // Keep the catalogue polyline intact. Stall regions and closed vector tips
    // can backtrack in either axis and must not be sorted or globally de-duplicated.
    return result;
  }
  ...
  function normalizePayload(payload={}){
    ...
    const sourcePoints=populatedPoints(model.sourcePoints,performance.sourcePoints,runtimePoints);
    const interpolation=model.interpolation||performance.interpolation||'';
    const precomputed=Boolean(model.precomputed??performance.precomputed);
    const rawControl=String(model.control||performance.control||'');
    const control=rawControl.toLowerCase()==='nominal'?'':rawControl;
    const rawControls=(performance.controls||[]).filter(Boolean);
    ...
    return {
      ...
      technical:{
        fireRating:technical.fireRating||model.fireRating||'',
        fanType:technical.fanType||model.fanType||'',
        mountType:technical.mountType||model.mountType||'',
        ipClass:technical.ipClass||model.ipClass||'',
        productCode:technical.productCode||model.productCode||'',
        availabilityRegion:technical.availabilityRegion||model.availabilityRegion||'',
        phase:technical.phase||model.phase||'',
        operatingTemperatureMinC:technical.operatingTemperatureMinC??model.operatingTemperatureMinC,
        operatingTemperatureMaxC:technical.operatingTemperatureMaxC??model.operatingTemperatureMaxC,
        continuousAirTemperatureC:technical.continuousAirTemperatureC??model.continuousAirTemperatureC,
        approximateContinuousAirTemperatureC:technical.approximateContinuousAirTemperatureC??model.approximateContinuousAirTemperatureC,
        atex:technical.atex||model.hazardousArea||null,
        safetyWarning:technical.safetyWarning||model.safetyWarning||''
      },
      performance:{nominalAirflow:performance.nominalAirflow??model.nominal,points:curvePoints(runtimePoints,sourcePoints,{interpolation,precomputed}),sourcePoints,control,controls,interpolation,precomputed},
