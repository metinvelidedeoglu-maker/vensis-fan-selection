(function(){
  ...
  function pointEditor(index,field,value){
    const q=value&&Number.isFinite(Number(value.q))?value.q:'';
  ...
  function priceEditor(index,item){return input(index,'price',num(item.price)||'',{type:'number',min:0,step:'0.01',unit:'€',label:'Unit price',compact:true})}
  ...
  function fanRow(item,index){
    ...
    return `<tr data-project-edit-row="${index}" data-project-kind="fan"><td>${productMarkup(item)}</td><td>${noteEditor(item,index)}</td><td>${pointEditor(index,'required',item.required)}</td><td>${pointEditor(index,'selected',item.selected)}</td><td>${input(index,'voltage',item.voltage||'',{label:'Voltage'})}</td><td>${input(index,'motorPower',num(item.motorPower)||'',{type:'number',min:0,step:'0.01',unit:'kW',label:'Power'})}</td><td>${input(index,'speed',num(item.speed)||'',{type:'number',min:0,step:1,unit:'rpm',label:'Speed'})}</td><td>${priceEditor(index,item)}</td><td>${discountEditor(index,item)}</td><td class="project-derived"><b>${hasPrice?money(net):'-'}</b></td><td>${quantityEditor(index,item)}</td><td class="project-derived"><b>${hasPrice?money(net*qty):'-'}</b></td><td class="project-inline-actions">${actions(index)}</td></tr>`;
  }
  function electricalRow(item,index){
    const net=netUnit(item);const qty=Math.max(1,Math.round(num(item.quantity)||1));const hasPrice=num(item.price)>0;
    return `<tr data-project-edit-row="${index}" data-project-kind="electrical"><td>${productMarkup(item)}</td><td>${noteEditor(item,index)}</td><td>${input(index,'power',item.power||'',{label:'Power'})}</td><td>${input(index,'lumen',item.lumen||'',{label:'Lumen'})}</td><td>${input(index,'voltage',item.voltage||'',{label:'Voltage'})}</td><td>${input(index,'ip',item.ip||'',{label:'IP rating'})}</td><td>${priceEditor(index,item)}</td><td>${discountEditor(index,item)}</td><td class="project-derived"><b>${hasPrice?money(net):'-'}</b></td><td>${quantityEditor(index,item)}</td><td class="project-derived"><b>${hasPrice?money(net*qty):'-'}</b></td><td class="project-inline-actions">${actions(index)}</td></tr>`;
  }
  ...
  function renderProjectTables(){
    ...
    ensureStyles();const items=store.readItems(id);const fan=[],electrical=[];
  ...
  function pointFromRow(row,field){
    ...
    return {q:Math.max(0,num(qText)),p:Math.max(0,num(pText))};
  }
  function updateItemFromRow(item,row){
    ...
    item.price=Math.max(0,num(valueOf(row,'price')));item.priceSource='manual';item.priceCurrency='EUR';
  ...
  function flushInlineEditors(){commitRows([...document.querySelectorAll('[data-project-edit-row]')])}
  ...
  function openProjectPrint(){
    ...
    localStorage.setItem(PRINT_KEY,JSON.stringify({version:6,createdAt:new Date().toISOString(),project:meta,items:items.map(technicalItem)}));
    window.open('project-print.html?print=1','_blank');
  }

  document.addEventListener('change',event=>{const control=event.target.closest('[data-project-inline]');if(control){const row=control.closest('[data-project-edit-row]');if(row)commitRows([row])}});
