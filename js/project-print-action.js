(function(){
  ...
  function productMarkup(item){
    ...
    const image=`<i class="project-cell-image-slot"${item.image?'':` aria-hidden="true"`}>${item.image?`<img src="${esc(item.image)}" alt="${esc(presentation.altModel||'Product')}" onerror="this.remove()">`:''}</i>`;
