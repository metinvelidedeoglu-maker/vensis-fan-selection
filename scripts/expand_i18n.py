...
path=Path('js/language.js')
text=path.read_text(encoding='utf-8')

extra=r'''  Object.assign(translations,{
...
marker='  const reverse=Object.create(null);'
if 'Expand Turkish application coverage' not in text:
    if marker not in text:
        raise RuntimeError('reverse marker not found')
    text=text.replace(marker,"  // Expand Turkish application coverage\n"+extra+marker,1)

dynamic=r'''  dynamicPairs.push(
...
dyn_marker='  function translatePattern(value,lang=currentLanguage){'
if 'dynamicPairs.push(' not in text:
    if dyn_marker not in text:
        raise RuntimeError('translatePattern marker not found')
    text=text.replace(dyn_marker,dynamic+dyn_marker,1)

dialog=r'''  function installDialogTranslation(){
...
selector_marker='  function selectorTarget(){'
if 'function installDialogTranslation()' not in text:
    if selector_marker not in text:
        raise RuntimeError('selector marker not found')
    text=text.replace(selector_marker,dialog+selector_marker,1)

text=text.replace('apply(document.body||document);','apply(document);')
text=text.replace('if(observer||!document.body)return;','if(observer||!document.documentElement)return;')
text=text.replace('observer.observe(document.body,{','observer.observe(document.documentElement,{')
text=text.replace("  function init(){\n    apply(document);", "  function init(){\n    installDialogTranslation();\n    apply(document);")
path.write_text(text,encoding='utf-8')

for p in [Path('js/project-nav.js'),Path('js/order-utils.js'),Path('detail.html'),Path('project-print.html')]:
    if p.exists():
        s=p.read_text(encoding='utf-8')
        s=s.replace('js/language.js?v=20260816-tr','js/language.js?v=20260816-tr2')
        p.write_text(s,encoding='utf-8')

for p in Path('.').glob('*.html'):
    s=p.read_text(encoding='utf-8')
    s=s.replace('js/project-nav.js?v=20260816-language','js/project-nav.js?v=20260816-language2')
    s=s.replace('js/order-utils.js?v=20260816-language','js/order-utils.js?v=20260816-language2')
    p.write_text(s,encoding='utf-8')
